...
- 操作类型：[重构]
- 影响文件：
  - `E:\myProjects_Trae\RadarIdentifySystem_Codex\.gitattributes`
  - `E:\myProjects_Trae\RadarIdentifySystem_Codex\resources\models\DTOA\02.09_ResNet_CA_Transformer_DTOA-final-73-accuracy1.00-val_accuracy0.99.onnx`
  - `E:\myProjects_Trae\RadarIdentifySystem_Codex\resources\models\PA\03.16_SparseMorphNet_PA-checkpoint-19-f10.98-val_f11.00.onnx`
  - `E:\myProjects_Trae\RadarIdentifySystem_Codex\docs\operateLog.md`
- 变更摘要：仅重写 `origin/newUI..newUI` 的本地提交，将两个默认 ONNX 模型从 Git LFS 指针转换为普通 Git 对象。
- 原因：仓库所有者的 GitHub LFS 预算已阻止推送，而两个模型单文件均低于普通 Git 的文件限制。
- 计划清单：
...
- 影响文件：
  ...
  - `E:\myProjects_Trae\RadarIdentifySystem_Codex\docs\operateLog.md`
- 变更摘要：修复合并重置会关闭合并菜单并把横向工作区自动滚回 A+B 的问题。
- 原因：重置只应清除当前切片合并计划、合并结果和合并呈现，不属于退出合并界面的导航操作；合并界面已打开时应保持 C+D，直到用户再次点击菜单退出。
- 计划清单：
  - [x] 核对重置工作流、控制器状态刷新和合并菜单到横向工作区的切换调用。
  - [x] 解耦重置后的候选可用性与当前合并界面打开状态。
  - [x] 保持 C+D、菜单选中及用户主动退出能力，同时清空合并结果与图像。
  - [x] 补充重置瞬间、后台重判完成和无候选结果的布局回归。
  - [x] 执行聚焦测试、Python 编译和差异检查。
- 验证结果：
  - 根因是重置后后台重新判别的短暂阶段没有候选和结果，控制器按旧规则取消合并菜单选中，进而触发工作区返回 A+B。
  - 合并菜单已选中时现在视为独立的用户导航状态；重置、后台重判及空结果都不会关闭菜单或改变 C+D，只有用户再次点击菜单才返回并锁定 A+B。
  - 重置仍只清除当前切片合并计划、合并结果、合并图像、来源显隐和结果表格，识别结果保持原对象不变。
  - 未打开合并界面且没有候选或结果时，菜单继续保持禁用，不会因本次导航保持规则被错误激活。
  - 重置瞬间、重判完成、空计划和用户主动退出聚焦回归：3 passed，1 个第三方 scipy 弃用警告。
  - 合并相关扩大回归：26 passed、7 deselected、3 failed；失败为既有合并结果标题“类/组”和结果数量空格文案断言漂移，与本次导航状态修复无关。
  - 相关 Python 文件 `py_compile` 与 `git diff --check`：通过（仅换行符提示）。
- 测试状态：[已测试]
