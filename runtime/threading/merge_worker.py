...
class MergeWorkerResult:
    ...
    Attributes:
        success [bool]: 策略判别和整批计算是否全部成功。
        plan [SliceMergePlan | None]: 策略生成的完整切片计划。
...
class MergeWorker(QThread):
    ...
    def __init__(
        ...
        Args:
            ...
            strategy [MergeStrategy]: 本次任务冻结的合并策略实例。
    ...
    def run(self) -> None:
        ...
        try:
            LOGGER.info(
                "合并线程开始策略判别: slice_index=%d, strategy_id=%s",
                self._slice_index,
            ...
            # 空计划是合法判别结果，无需进入点云和绘图计算。
            if not plan.groups:
                self.finished_signal.emit(
                    self._session_id,
                    MergeWorkerResult(success=True, plan=plan),
                )
                return
